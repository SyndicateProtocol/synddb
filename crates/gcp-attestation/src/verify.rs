...
pub fn verify_attestation(
    ...
    if parsed.header.alg != "RS256" {
        return Err(VerificationError::UnsupportedAlgorithm(parsed.header.alg));
    }

    // Verify key ID matches
    if parsed.header.kid != jwk.kid {
        return Err(VerificationError::KeyNotFound(parsed.header.kid));
    }
    ...
    if parsed.claims.iss != "https://confidentialcomputing.googleapis.com" {
        return Err(VerificationError::InvalidIssuer(parsed.claims.iss));
    }

    // Validate software name if present
    if let Some(swname) = &parsed.claims.swname {
        if swname != "CONFIDENTIAL_SPACE" {
            return Err(VerificationError::InvalidSwname(swname.clone()));
        }
    }
    ...
    if let Some(now) = current_time {
        ...
        if parsed.claims.nbf > 0 && now < parsed.claims.nbf {
    ...
    Ok(ValidationResult {
        ...
        image_digest: parsed.claims.image_digest().unwrap_or_default().to_string(),
        secboot: parsed.claims.secboot,
        audience: parsed.claims.aud.clone(),
        nonce: parsed.claims.eat_nonce.clone(),
        dbgstat: parsed.claims.dbgstat.clone().unwrap_or_default(),
        hwmodel: parsed.claims.hwmodel.clone().unwrap_or_default(),
    })
}
...
mod tests {
    ...
    fn test_verify_attestation() {
        ...
        let result = verify_attestation(SAMPLE_JWT, &jwk, None, None).unwrap();
    ...
    fn test_verify_with_audience() {
        ...
        let result = verify_attestation(SAMPLE_JWT, &jwk, Some("https://wrong.com"), None);
        assert!(matches!(
            result,
    ...
    fn test_wrong_key() {
        ...
        jwk.kid = "wrong-kid".to_string();

        let result = verify_attestation(SAMPLE_JWT, &jwk, None, None);
