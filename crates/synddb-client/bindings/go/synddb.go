...
extern SyndDBError synddb_attach_with_config(const char* db_path, const char* sequencer_url, uint64_t push_interval_ms, uint64_t snapshot_interval, SyndDBHandle** out_handle);
...
type Config struct {
	// PushIntervalMs is the milliseconds between automatic changeset pushes (default: 1000)
	PushIntervalMs uint64
...
func AttachWithConfig(dbPath, sequencerURL string, config Config) (*Handle, error) {
	...
	result := C.synddb_attach_with_config(
		dbPathC,
		urlC,
		C.uint64_t(config.PushIntervalMs),
