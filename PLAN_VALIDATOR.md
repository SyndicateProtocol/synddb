...
/// Bridge signer - signs withdrawals and state attestations
/// Enabled with --bridge-signer flag
pub struct BridgeSigner {
    signer: PrivateKeySigner,
    bridge_contract: Address,
    withdrawal_signer: WithdrawalSigner,
    state_attestor: StateAttestor,
    signature_store: SignatureStore,
}

impl BridgeSigner {
    pub async fn new(config: &ValidatorConfig) -> Result<Self> {
        let signing_key = config.bridge_signing_key.as_ref()
            .ok_or_else(|| anyhow!("--bridge-signing-key required with --bridge-signer"))?;

        let signer: PrivateKeySigner = signing_key.parse()
            .context("Invalid bridge signing key")?;

        let bridge_contract: Address = config.bridge_contract.as_ref()
            .ok_or_else(|| anyhow!("--bridge-contract required with --bridge-signer"))?
            .parse()
        ...
        Ok(Self {
            signer,
            bridge_contract,
            withdrawal_signer: WithdrawalSigner::new(),
            state_attestor: StateAttestor::new(),
            signature_store,
        })
    }

    /// Called when a withdrawal message is applied
    pub async fn on_withdrawal(&self, withdrawal: &WithdrawalRequest, sequence: u64) -> Result<()> {
        let signature = self.withdrawal_signer.sign(&self.signer, withdrawal, sequence)?;

        self.signature_store.store_withdrawal_signature(
    ...
    /// Called periodically to attest to current state root
    pub async fn attest_state(&self, state_root: B256, sequence: u64) -> Result<()> {
        let signature = self.state_attestor.sign(&self.signer, state_root, sequence)?;

        self.signature_store.store_state_attestation(
            sequence,
            signature,
        )?;

        debug!(sequence = sequence, "State attested");
        Ok(())
    }
}

// src/bridge/withdrawal_signer.rs

/// Signs withdrawal approvals for the bridge contract
...
impl WithdrawalSigner {
    ...
    /// Sign a withdrawal approval
    ///
    /// Message format (EIP-712 style):
    ...
    pub fn sign(
        ...
        sequence: u64,
    ) -> Result<WithdrawalSignature> {
        // Encode the withdrawal approval message
        let message = self.encode_withdrawal_message(withdrawal, sequence);
        let message_hash = keccak256(&message);

        // Sign with Ethereum prefix
        let signature = signer.sign_hash_sync(&message_hash)
            .context("Failed to sign withdrawal")?;

        Ok(WithdrawalSignature {
            request_id: withdrawal.request_id.clone(),
            recipient: withdrawal.recipient.parse()?,
            amount: withdrawal.amount.parse()?,
            sequence,
    ...
    fn encode_withdrawal_message(&self, withdrawal: &WithdrawalRequest, sequence: u64) -> Vec<u8> {
        // ABI encode for EIP-712 compatible signing
        let mut encoded = Vec::new();
        encoded.extend_from_slice(b"WithdrawalApproval");
        encoded.extend_from_slice(withdrawal.request_id.as_bytes());
        // ... encode recipient, amount, sequence
        encoded
    }
}
...
// src/bridge/signature_store.rs

/// Stores signatures for relayers to retrieve
pub struct SignatureStore {
    withdrawal_signatures: DashMap<String, Vec<WithdrawalSignature>>,
    state_attestations: DashMap<u64, Vec<StateAttestation>>,
}

impl SignatureStore {
    pub fn new() -> Result<Self> {
        Ok(Self {
            withdrawal_signatures: DashMap::new(),
            state_attestations: DashMap::new(),
        })
    }

    pub fn store_withdrawal_signature(&self, request_id: &str, sig: WithdrawalSignature) -> Result<()> {
        self.withdrawal_signatures
    ...
    /// Get all signatures for a withdrawal (called by relayers)
    pub fn get_withdrawal_signatures(&self, request_id: &str) -> Vec<WithdrawalSignature> {
        self.withdrawal_signatures
            .get(request_id)
            .map(|sigs| sigs.clone())
            .unwrap_or_default()
    ...
    pub fn store_state_attestation(&self, sequence: u64, attestation: StateAttestation) -> Result<()> {
...
// src/bridge/api.rs - Signature API for relayers

/// API for relayers to retrieve signatures
pub fn signature_router(store: Arc<SignatureStore>) -> Router {
    Router::new()
...
async fn get_withdrawal_signatures(
